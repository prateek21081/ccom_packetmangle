...
//  }
  ...
  if (!data.pipeline || !data.pulsesrc || !data.audioconvert || !data.opusenc || !data.rtpopuspay || !data.udpsink) {
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.pulsesrc, data.audioconvert, data.opusenc, data.rtpopuspay, data.udpsink, NULL);
  if (!gst_element_link_many (data.pulsesrc, data.audioconvert, data.opusenc, data.rtpopuspay, data.udpsink, NULL)) {
 ...
 // g_object_set (data.fdsink, "fd", FIFO_FD, NULL);
  g_object_set (data.udpsink, "host", "192.168.226.165", NULL);
  g_object_set (data.udpsink, "port", 5000, NULL);
  g_object_set (data.udpsink, "sync", false, NULL);
  g_object_set (data.udpsink, "async", false, NULL);
