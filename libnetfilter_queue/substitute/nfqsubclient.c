...
static int callback(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg, struct nfq_data *nfa, void *data) {
    ...
    unsigned char modified_pkt[pkt_len];
    ...
    unsigned char mod_msg[] = "Hello";
    unsigned int offset = sizeof(struct iphdr) + sizeof(struct udphdr) + 24;
    int found_mod_msg = 1;
    for (int i = 0; i < 5; i++) {
        printf("%c", modified_pkt[offset + i]);
        if (modified_pkt[offset + i] != mod_msg[i]) {
            found_mod_msg = 0;
            // break;
        }
    }
    printf("\n");
    if (found_mod_msg) {
        printf("%d mod_msg found, modifying it...\n", ip_hdr->id);
        modified_pkt[offset] = 'M';
    }
    ...
    return nfq_set_verdict(qh, ntohl(ph->packet_id), NF_ACCEPT, pkt_len, modified_pkt);
}
